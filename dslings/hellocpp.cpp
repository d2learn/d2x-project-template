...
//    运行时检查点后, 你可以删除或注释掉代码中的 d2x::wait() 函数调用, 会自动进入下一个练习.
//
//      - d2x::wait: 该函数用于隔离不同练习, 你可以删除或注释掉该函数调用, 进入下一个练习.
//      - d2x::assert_eq: 该函数用于运行时检查点, 你需要修复代码中的错误, 使得所有检查点通过
//      - D2X_YOUR_ANSWER: 该标识用于提示你需要修改的代码, 一般用于代码填空(即用正确的代码替换这个标识)
//
// Auto-Checker/自动检测命令:
//
...
int main() {
    ...
    d2x::wait(); // 5.删除或注释该调用, 进入下一个练习(项目正式代码练习)
